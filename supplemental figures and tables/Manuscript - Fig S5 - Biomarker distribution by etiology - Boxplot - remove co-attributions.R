...
# this is a supplemental figure that is removing co-attributions from each of the pathogens. 
...
pathogen_function <- function(df, pathogen_attributable, pathogen_name) {
  ...
  # list of all pathogen attribution columns
  pathogen_cols <- c(
  ...
  # identify all other pathogens
  other_pathogens <- setdiff(pathogen_cols, pathogen_attributable)
  
  result <- df %>%
    filter(
      !!sym(pathogen_attributable) == 1,  # focal pathogen is present
      rowSums(across(all_of(other_pathogens))) == 0) %>% # no co-attributions
    select(pid, !!sym(pathogen_attributable), biomarker, log10_conc) %>%
    mutate(pathogen = pathogen_name) %>%
    rename(pathogen_present = !!sym(pathogen_attributable))
...
small <- long_df %>% filter(
  shigella_attributable == 0,
  tEPEC_attributable == 0,
  c_jejuni_coli_attributable == 0,
  ST.ETEC_attributable == 0,
  cryptosporidium_attributable == 1,
...
# Save Figure 
ggsave(file="Figures/Figure SX - Distribution of biomarkers by diarrhea etiology - remove co-attributions - FINAL.pdf",plot,width=12,height=6,dpi=300)
