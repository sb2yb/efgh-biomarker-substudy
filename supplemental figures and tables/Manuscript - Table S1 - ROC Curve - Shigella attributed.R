...
haem <- EFGH_substudy_rowperpid_ALLDATA %>%
  select(pid, clean_conc_log10_HAEM, country, dysentery, shigella_attributable) %>%
...
# x = continuous biomarker (predictor)
# y = dichotomous dysentery (outcome)
# cutpointr(data, x, y)
...
h <- ggplot(data=rocplot,aes(x=fpr,y=tpr)) +
  geom_line() +
  geom_point(aes(x=0.266932,y=0.594286),size=3, color="red") # x = 1- specificity y = sensitivity 
h = h + theme_bw()
...
# save figure
# ggsave(file="Figures/Figure 2 - HAEM and dysentery ROC curve - Shigella attributed - FINAL.pdf",h,width=8,height=6,dpi=300)
