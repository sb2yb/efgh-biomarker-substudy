# This is "the answer code" for Figure 1 density plots by dysentery variable: shigella detected NOT attributed
...
table(EFGH_substudy_rowperpid_ALLDATA$shigella_detect, useNA= "always")
...
table(plot1_df$shigella_ct, useNA = "always")
...
########
  ...
  blood_plot = blood_plot + scale_fill_manual(values=c("#AC1408", "#09B9FF"),
                                              ...
                                              breaks = c(0,1))
  blood_plot = blood_plot + theme(legend.position = c(0.85, 0.80))
  ...
  blood_plot = blood_plot + xlim(-7.5, 4.5) # Set x-axis range to go from 0 to 5
  blood_plot

  ggsave(file="Figures/Figure S3 - HAEM density - Shigella detected NOT attributed - FINAL.pdf",blood_plot,width=10,height=6,dpi=300)
