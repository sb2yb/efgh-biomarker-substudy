...
# Sensitivity Analysis: As a comparison for children with multiple enrollments vs. one enrollment, we will remove all children with more than 1 enrollment and re-run all analyses as a “sensitivity analysis” to see the potential effect that children enrolled more than once have on the results. 
...
df <- EFGH_substudy_rowperpid_ALLDATA %>%
  select(pid, child_id, scr_date, country, clean_conc_log10_CAL, clean_conc_log10_HAEM, clean_conc_log10_MPO, clean_conc_log10_NGAL, shigella_ct,
...
# remove duplicates - keep first visit and remove subsequent visits 
# filtering out duplicates should happen AFTER we move log10 of 0 and NAs because we want to keep the first instance of pathogen specific diarrhea if possible -
# if we remove duplciates first it's possible the first visit was a non-detected pathogen specific diarrhea and then we would lose that kid entirely even if they have an episode later
shigella_df <- shigella_df %>%
  group_by(child_id) %>%
  arrange(child_id, scr_date) %>% # sorts the data by scr_date (oldest first) for each child_id
  slice(1) %>% # takes the first row for each group (the one with the oldest date)
  ungroup()
...
# create function to go through biomarkers and log10 values of pathogens
# remove gee components since it's no longer repeated measurements 
risk_func <-function(biomarker, pathogen, dat) {
  model <- glm(biomarker ~ pathogen + country, family = gaussian, data = dat)
...
# save table
write.csv(biomarkers, "Figures/Table S4 - Pathogen and Biomarker Mean Differences - Sensitivity Analysis - FINAL.csv")
