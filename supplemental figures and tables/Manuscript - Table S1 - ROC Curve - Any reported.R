...
haem <- EFGH_substudy_rowperpid_ALLDATA %>%
  select(pid, clean_conc_log10_HAEM, country, dysentery) %>%
...
h <- ggplot(data=rocplot,aes(x=fpr,y=tpr)) +
  geom_line() +
  geom_point(aes(x=0.185159,y=0.509831),size=3, color="red") # x = 1- specificity y = sensitivity 
h = h + theme_bw()
...
h = h + labs(title = "Log 10 concentration of hemoglobin vs. observed blood in stool", y="Sensitivity", x="1-Specificity")+ theme(axis.text = element_text(colour = "black"))
h


# save figure
# ggsave(file="Figures/Figure 2 - HAEM and dysentery ROC curve - Any reported - FINAL.pdf",h,width=8,height=6,dpi=300)
